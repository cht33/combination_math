...
![result_fig](fig/result.png)

在去掉输出时间的情况下，对上述报告中提到的所有算法做了性能的测试。

四种课程内算法(优化后)性能从高到低排序大致为邻位对换937.50ms,递减进制数1078.12ms，字典序1156.25ms，递增进制数1796.88ms，均快于STL版本的1937.50ms。

另外针对各个版本的调优和重新设计，性能提升也很明显。

increase_base: 48109.38ms -> 1796.88ms

decrease_base: 49015.62ms -> 1078.12ms

neighbour_exchange: 71453.12ms -> 937.50ms

recursive: 14484.38ms -> 4421.88ms（优化前后排列顺序不同，详见新式递归法部分）
...
- run.sh
  ...
  编译运行脚本，控制台`sh run.sh`运行，输入目标排列长度`n`即开始运行所有算法，效果与上部分截图一致。(编译后会生成build文件夹存放可执行文件)
